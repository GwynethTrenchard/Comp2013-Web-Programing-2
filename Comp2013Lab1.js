...
const realtorFees = (listingPrice) => {
  let realtorFeePercentage;
  listingPrice = listingPrice.replace(/\D/g, "");
  // i found ".replace(/\D/g,'');" on stack overflow, it is used to remove all non-digit characters from a string.
  // https://stackoverflow.com/questions/1862130/strip-all-non-numeric-characters-from-string-in-javascript

  listingPrice > 450000
    ? (realtorFeePercentage = 0.02)
    : (realtorFeePercentage = 0.025);
  let realtorFee = listingPrice * realtorFeePercentage;

  return realtorFee;
};

let realatorCost = realtorFees(listings[2].price);

console.log(realatorCost);
...
const newBuiltListings = listings.filter(
  (listing) =>
    listing.propertySummary.builtIn.includes("199") ||
    listing.propertySummary.builtIn.includes("20")

  // i hope you know u spelt builtIn wrong and i almost lost my mind trying to figure out what i was doing wrong ;-;
  // i was on reddit of all places trying to fix it
  // its fine im just tired
);
